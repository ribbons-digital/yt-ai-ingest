...
`ytai teach <folder> <topic-id> --refresh` requires an existing lesson and explicit topic id, writes a repair-oriented `learning/lessons/<nn>-<id>-input.md` that embeds the current lesson plus validation warnings, and never overwrites the lesson file itself.
