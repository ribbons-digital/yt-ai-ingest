...
The Markdown lesson remains canonical; the HTML renderer supports a safe Markdown subset, escapes lesson content, emits no scripts, uses native `<details>` for practice answers, renders matching concept cards, and shows existing local frame evidence as thumbnails.
