...
`ytai teach <folder> <topic-id>` (or `--next` for the next unfinished topic in teaching order) writes `learning/lessons/<nn>-<id>-input.md` with topic-scoped transcript excerpts padded 15 seconds on each side; the prompt embeds `learner-profile.json`, `teaching-guide.md`, only concept cards from `concepts.json` whose `neededForTopics` includes the current topic id, and the matching topic section from `resources.md`; missing artifacts are called out in the prompt so the LLM proceeds cautiously instead of failing.
An LLM answers with `learning/lessons/<nn>-<id>.md`.
`ytai teach <folder> <topic-id> --refresh` requires an existing lesson and explicit topic id, cannot be combined with `--next`, writes a repair-oriented `learning/lessons/<nn>-<id>-input.md` that embeds the current lesson plus validation warnings, and never overwrites the lesson file itself.
Regenerating or refreshing a lesson prompt for a done topic resets its status to pending while preserving existing quiz scores and next review time.
`ytai render-lesson <folder> <topic-id>` reads the existing Markdown lesson and writes `learning/lessons/<nn>-<id>.html`.
The Markdown lesson remains canonical; the HTML renderer supports a safe Markdown subset, escapes lesson content, emits no scripts, uses native `<details>` for practice answers, renders matching concept cards, and shows existing topic timestamps plus local frame evidence as thumbnails.
...
With `--dry-run`, `ytai topics`, `ytai plan`, `ytai teach`, `ytai render-lesson`, `ytai quiz`, `ytai score`, and `ytai learn --done` validate inputs and print the prompt file, HTML file, or progress update they would make without writing prompt files, HTML files, or changing `learning/progress.json`.
...
Stages: no-context -> needs-topics-input -> awaiting-topics -> (topics-invalid) -> needs-plan-input -> awaiting-plan -> teaching -> complete; complete requires every core and supporting lesson done, tangent topics are optional. New folders stay in awaiting-plan until `plan.md`, `resources.md`, and a concept-valid `concepts.json` exist; legacy folders with existing lesson/progress state do not move backwards solely because `concepts.json` predates the feature or has validation errors.
