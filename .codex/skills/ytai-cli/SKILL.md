...
- `src/commands/learn.ts`: I/O layer for `topics`/`plan`/`teach`/`render-lesson`/`learn`/`score`: writes `learning/*-input.md` prompts, writes `learning/teaching-guide.md` once when topics are generated, reads and validates `learning/topics.json`, tracks `learning/progress.json`, builds topic transcript excerpts (ranges padded 15s each side), handles `teach --refresh` repair prompts for existing lessons, and renders completed lessons to local HTML. `plan` now asks the LLM to write `plan.md`, `resources.md`, and `concepts.json`; `recordScore` appends quiz scores and schedules reviews; exports `recordScore`, `requireValidTopics`, `readProgress`, and `buildTranscriptExcerpt` (reused by `quiz.ts`).
...
- `src/lib/lessonHtml.ts`: safe local HTML lesson renderer used by `ytai render-lesson`; keeps Markdown canonical, emits no scripts, escapes lesson content, and renders concept cards plus available topic timestamps and frame thumbnails.
...
| `--refresh` | teach | Create a repair prompt for an existing lesson; requires an explicit topic id |
...
| `--dry-run` | global | Preview external commands and write actions; ingest/prepare skip folder prompts, learning commands validate without writing prompt files, HTML files, or progress updates, and transcribe previews `mlx_whisper` plus `ffmpeg` without requiring `audio.wav` |
...
- Learning round-trip contract: ytai never calls AI. `topics` writes `topics-input.md` and creates `teaching-guide.md` only when missing; `plan` writes `plan-input.md`; `teach` writes lesson prompts; `teach --refresh` writes a repair prompt that embeds the current lesson plus validation warnings without overwriting the lesson file; any LLM writes `topics.json`, `plan.md`, `resources.md`, `concepts.json`, and `lessons/<nn>-<id>.md`; `render-lesson` renders a completed Markdown lesson to self-contained local HTML without making HTML canonical; `learn` validates artifacts and reports the next action (`kind: "cli" | "llm"`). With `--dry-run`, `topics`, `plan`, `teach`, `render-lesson`, `quiz`, `score`, and `learn --done` validate and print the prompt file, HTML file, or progress update they would make without writing prompt files, HTML files, or changing `progress.json`.
...
- Regenerating or refreshing a lesson prompt for a done topic resets its status to `pending` while preserving existing quiz `scores` and `nextReviewAt` fields.
...
- Lesson HTML (`ytai render-lesson <folder> <topic-id>`): reads `learning/lessons/<nn>-<id>.md`, renders a safe Markdown subset to `learning/lessons/<nn>-<id>.html`, escapes lesson content, emits no scripts, uses native `<details>` for practice answers, includes matching concept cards, and shows available topic timestamps plus frame evidence as local thumbnails.
