...
- `src/lib/lessonHtml.ts`: safe local HTML lesson renderer used by `ytai render-lesson`; keeps Markdown canonical, emits no scripts, escapes lesson content, and renders concept cards plus available frame thumbnails.
...
- Lesson HTML (`ytai render-lesson <folder> <topic-id>`): reads `learning/lessons/<nn>-<id>.md`, renders a safe Markdown subset to `learning/lessons/<nn>-<id>.html`, escapes lesson content, emits no scripts, uses native `<details>` for practice answers, includes matching concept cards, and shows available frame evidence as local thumbnails.
